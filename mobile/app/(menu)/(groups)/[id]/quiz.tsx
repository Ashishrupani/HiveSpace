...
export default function AIPage() {
  ...
  const generateSummary = async (content: string) => {
    setLoading(true);
    try {
      // ragApi wraps this as { note: { text: content } } internally
  ...
  const generateQuiz = async (content: string, options: QuizOptions) => {
    setLoading(true);
    try {
      // ragApi wraps this as { note: { text: content }, options } internally
  ...
  const handleSaveQuiz = async () => {
    ...
    setSavingQuiz(true);
    try {
      // Sends { quiz: quizData, score?: quizScore } — matches saveQuizHandler
  ...
  const handleSaveSummary = async () => {
    ...
    try {
      // Sends { summary: summaryData } — matches saveSummaryHandler
    ...
    return (
      <View style={styles.container}>
        ...
        <QuizComponent
          ...
          // If your QuizComponent exposes a score callback, wire it here:
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.contentPadding}>
      ...
      <View style={styles.counterCard}>
        <Ionicons
          name="flash"
          size={18}
          color={generationsRemaining > 0 ? GREEN : RED}
        />
        <Text style={[
          styles.counterText,
          generationsRemaining === 0 && styles.counterTextZero
        ]}>
...
const styles = StyleSheet.create({
  ...
  // Shared screen header (quiz + summary views)
  screenHeader: {
    flexDirection: 'row',
    alignItems: 'center',
